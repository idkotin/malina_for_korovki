...
Спящий режим при простое пропускает только обычные отправки телеметрии. GPS, вес, события модема и досылка буфера продолжают работать в обычном цикле, поэтому короткие скачки скорости GPS не будят отправку сразу.

Чтение веса, опрос Wi-Fi, HTTP-отправка и выгрузка SQLite-буфера выполняются в отдельных фоновых потоках. Медленный ADS1263 и большая очередь старых пакетов больше не уменьшают частоту создания свежей телеметрии. Ошибка Wi-Fi-опроса очищает прежний список клиентов; резервный `ip neigh` не учитывает записи `STALE` и `FAILED`.
